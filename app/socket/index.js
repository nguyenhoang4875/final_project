...
const ioEvents = function (io) {
	...
	io.of('/rooms').on('connection', function (socket) {
		...
		// Create a new room
		socket.on('createRoom', async function ({name, members, id}) {
			const newRoom = await RoomService.create({name, members, id});
	...
	io.of('/users').on('connection', function (socket) {
		...
		socket.on('editUser', async function ({ name, email, id, role, oldPassword, newPassword, userId }) {
			const data = await UserService.updateUser({username: name, email, id, role, oldPassword, newPassword, userId });
			socket.emit('updateUsersList', data, id);
			socket.broadcast.emit('updateUsersList', data, id);
		});
